...
class MIDI_Gadget(ControlSurface):   # Make sure you update the name
    __doc__ = " Script for MIDI_Gadget in APC emulation mode "   # Make sure you update the name
    ...
    def _combine_active_instances():
        track_offset = 0
        scene_offset = 0
        for instance in MIDI_Gadget._active_instances:   # Make sure you update the name
    ...
    def __init__(self, c_instance):
        ...
        self._do_combine()
        #to update all params on board at start:
        #self._on_volume_changed.subject = self.song().tracks[0].mixer_device.volume
        #self._on_volume_changed.subject = self.song().tracks[1].mixer_device.volume
        #self._on_volume_changed()
        for i, track in enumerate(self.song().tracks[:8]):
            volume = int(track.mixer_device.volume.value * 127)  # normalized to 0-127
            mute = int(track.mute)
            solo = int(track.solo)
            pan = int((track.mixer_device.panning.value + 1) * 63.5)  # Map -1.0–1.0 to 0–127
            arm = int(track.arm if track.can_be_armed else 0)
            # Example MIDI format (replace with your actual mapping)
            # Send CCs or SysEx as needed by your controller
            self._send_midi((0xB0, 0x00 + i, volume))  # Channel 1, CC 0–7 for volume
            self._send_midi((0xB0, 0x08 + i, pan))     # Pan: CC 8–15, Ch 1
            self._send_midi((0xB1, 0x00 + i, mute))    # Channel 2, CC 0–7 for mute
            self._send_midi((0xB1, 0x08 + i, solo))    # Channel 2, CC 8–15 for solo
            self._send_midi((0xB1, 0x10 + i, arm))     # Channel 2, CC 16–23 for arm    
...
# todo: update 
    ...
    def _on_volume_changed(self):
        value = self.song().tracks[0].mixer_device.volume.value
        midi_value = int(value * 127)  # Scale 0.0–1.0 to 0–127
        self._send_midi((0xB0, 0x00, midi_value))  # CC 0 = Volume on MIDI channel 1
    ...
    def _do_combine(self):
        if self not in MIDI_Gadget._active_instances:    # Make sure you update the name
            MIDI_Gadget._active_instances.append(self)   # Make sure you update the name
            MIDI_Gadget._combine_active_instances()  # Make sure you update the name

    def _do_uncombine(self):
        if (self in MIDI_Gadget._active_instances) and MIDI_Gadget._active_instances.remove(self):    # Make sure you update the name
